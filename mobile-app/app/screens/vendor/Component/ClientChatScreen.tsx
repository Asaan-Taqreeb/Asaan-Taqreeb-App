...
export default function ClientChatScreen() {
    ...
    useEffect(() => {
        ...
        const handleReceiveMessage = (newMessage: Message) => {
            ...
            if (!normalizedMessage._id || !normalizedMessage.senderId || !normalizedMessage.receiverId) return
    ...
    const handleSendInvoice = async () => {
        const cleanedTotal = String(invoiceTotal || '').replace(/[^0-9.]/g, '')
        const cleanedAdvance = String(invoiceAdvance || '0').replace(/[^0-9.]/g, '')

        const total = Number(cleanedTotal)
        const advance = Number(cleanedAdvance)

        if (!cleanedTotal || isNaN(total) || total <= 0) {
            showAlert('Invalid Total', 'Please enter a valid total booking amount.')
            return
        }
        if (isNaN(advance) || advance < 0 || advance > total) {
            showAlert('Invalid Advance', 'Advance payment cannot exceed the total amount.')
            return
        }

        const targetClientId = clientId || dynamicClientId
        const targetChatId = chatId || (user?.id && targetClientId ? buildChatId(targetClientId, user.id) : '')

        if (!targetClientId || !targetChatId) {
            showAlert('Error', 'Cannot send invoice: Client ID or Chat context missing.')
            return
        }
...
Generated by ${user?.name || 'Vendor Partner'}`
        ...
        const optimisticMessage: Message = {
            ...
            chatId: targetChatId,
            senderId: { _id: user?.id as string, name: user?.name as string, email: '' },
            receiverId: { _id: targetClientId, name: clientName, email: '' },
            text,
            isRead: false,
            createdAt: new Date().toISOString()
        }
        ...
        try {
            const savedMessage = normalizeMessage(await sendMessage(targetChatId, targetClientId, text))
            setMessages(prev => {
                if (prev.some(m => m._id === savedMessage._id)) {
                    return prev.filter(m => m._id !== optimisticMessage._id)
                }
        ...
        return (
            <View style={StyleSheet.flatten([styles.container, {paddingTop: insets.top, paddingBottom: insets.bottom}]) as ViewStyle}>
                <View className='flex-1 justify-center items-center py-20'>
                    ...
                    <Text className='text-sm font-bold mt-4' style={{color: Colors.textSecondary}}>Loading chat...</Text>
                </View>
            </View>
        )
    }
